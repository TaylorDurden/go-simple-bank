...
func TestJWTAuth(t *testing.T) {
	authenticator, err := NewJWTAuthenticator(util.RandomString(32))
...
func TestExpiredJWTToken(t *testing.T) {
	auth, err := NewJWTAuthenticator(util.RandomString(32))
	require.NoError(t, err)

	token, payload, err := auth.CreateToken(util.RandomOwner(), -time.Minute)
	require.NoError(t, err)
	require.NotEmpty(t, token)
	require.NotEmpty(t, payload)
